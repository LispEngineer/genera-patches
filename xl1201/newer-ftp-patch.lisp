...
;;; Reason: Flavor TCP::TCP-FTP-SERVER:  new instance variable for last host seen
;;; Function (DEFUN-IN-FLAVOR TCP::FTP-SERVER-PATHNAME TCP::TCP-FTP-SERVER):  set and use new variable
;;; Written by reti, 6/14/07 06:42:34
...
;;; Allow access to fep via ftp (from W:>RETI>new-ftp-fep-patch.lisp.1).
...
(SCT:NOTE-PRIVATE-PATCH "Sticky host for FTP")
...
(defun-in-flavor (ftp-server-pathname tcp-ftp-server) (args)
  (if (and (stringp args)
	...
	(setq last-ftp-host (scl::send answer :host))
	answer)
      (fs:merge-pathnames (fs:parse-pathname args last-ftp-host) working-dir)))
